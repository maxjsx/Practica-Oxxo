...
def clean_text(x):
    if pd.isna(x): return ""
    # REGLA 1: Borrar dobles espacios para que SAP y Talana hagan match perfecto
    return re.sub(r'\s+', ' ', str(x).strip().upper())

def to_float_safe(val):
    if pd.isna(val) or str(val).strip() == "": 
        return np.nan # Devuelve nulo si está vacío, para no confundirlo con un 0 real
...
print("🚀 Iniciando FTE V16.0 (Regla SAP->Talana + Filtros Exactos)...")
...
# REGLA 3: Mantener los Ceros Literales
...
# REGLA 2: Cualquier permiso > 15 días (Licencia, Post Natal, Accidente) elimina el FTE
ruts_licencia_larga = set(df_perm[
    (df_perm["DURACION"] > 15) & 
    (df_perm["F_INI"] <= FECHA_CORTE) & 
    (df_perm["F_FIN"] >= FECHA_CORTE)
]["RUT_KEY"])

# ==========================================
# 5. CONSTRUCCIÓN DE LA SÁBANA Y FALLBACK
# ==========================================
print("Aplicando regla estricta: SAP primero, Talana si es nulo...")
...
# --- REGLA SEGÚN TU DIAGRAMA DE FLUJO ---
def definir_cargo_oficial(row):
    sap = clean_text(row["CARGO_SAP"])
    tal = clean_text(row["Cargo"])
    
    # Si SAP existe y no es nulo/vacío, se usa SAP
    if pd.notna(row["CARGO_SAP"]) and sap != "" and sap != "NAN":
        return sap
    
    # De lo contrario, se usa Talana
    return tal

df_final["CARGO OFICIAL"] = df_final.apply(definir_cargo_oficial, axis=1)
# --------------------------------------

# Ahora pasa por el agrupador
df_final["Agrupador"] = df_final["CARGO OFICIAL"].map(map_cargo_to_agrupa)
df_final["Agrupador"] = df_final["Agrupador"].fillna(df_final["CARGO OFICIAL"])
df_final["FTE TEORICO"] = df_final["CARGO OFICIAL"].map(map_cargo_to_fte).fillna(0.0)

# Inclusiones y cálculo real (Respetando los Ceros)
