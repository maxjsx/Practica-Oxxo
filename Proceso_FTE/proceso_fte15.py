...
print("🚀 Iniciando FTE V14.0 (Blindaje de Base y Rescate SAP)...")
...
# --- LÓGICA DE BLINDAJE Y RESCATE ---
def definir_cargo_estricto(row):
    tal = str(row["Cargo"]).strip().upper()
    sap = str(row["CARGO_SAP"]).strip().upper()

    # 1. BLINDAJE: Si Talana ya es un cargo válido, se queda intacto (Asegura los 1441 pts)
    if tal in cargos_validos:
        return tal
    
    # 2. RESCATE: Si Talana NO es válido, vemos si SAP puede salvar el FTE
    # Solo rescatamos si Talana no menciona nada de horas o part time
    tal_is_pt = any(keyword in tal for keyword in ["PT", "PART", "HRS", "HORAS"])
    
    if sap in cargos_validos and not tal_is_pt:
        return sap
        
    # 3. Si no se pudo rescatar, devolvemos Talana (quedará con FTE 0)
    return tal
...
# --- DE VUELTA LA REGLA DE ORO DE LOS CEROS ---
df_final["fte incls"] = df_final["RUT_KEY"].map(dict_fte_incl)
df_final["fte incls"] = df_final["fte incls"].replace(0, np.nan) # ¡Esta línea nos devuelve la precisión!
