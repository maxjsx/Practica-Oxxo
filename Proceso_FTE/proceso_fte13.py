...
print("🚀 Iniciando FTE V12.0 (Lógica Invertida: Talana -> SAP)...")
...
# Diccionario maestro para saber si un cargo es válido o "tira error"
...
# ==========================================
# ... (El inicio se mantiene igual hasta df_perm) ...

# CAMBIO 1: Licencias de 15 días o más (>= 15 en lugar de > 15)
...
# ==========================================
# 5. CONSTRUCCIÓN DE LA SÁBANA Y FALLBACK ESTRICTO
# ==========================================
print("Aplicando lógica estricta de clonación de FTE...")
...
# --- CAMBIO 2: LÓGICA DE RESCATE RESTRINGIDA ---
def definir_cargo_estricto(row):
    tal = str(row["Cargo"]).strip().upper()
    sap = str(row["CARGO_SAP"]).strip().upper()

    # Si el cargo de Talana es válido y conocido, se usa.
    if tal in cargos_validos:
        return tal
    
    # Si Talana dice "PART TIME" genérico, NO dejamos que SAP le regale un Full-Time.
    tal_is_pt = "PT" in tal or "PART TIME" in tal
    sap_is_pt = "PT" in sap or "PART TIME" in sap
    
    if tal_is_pt and not sap_is_pt:
        return tal # Forzamos a que devuelva el error original de Talana (FTE 0)
    
    # Si no había conflicto de Part-Time, usamos SAP como rescate
    if sap in cargos_validos:
        return sap
        
    return tal

df_final["CARGO OFICIAL"] = df_final.apply(definir_cargo_estricto, axis=1)
# --------------------------------------
...
# --- CAMBIO 3: RESPETAR LOS CEROS MANUALES ---
# Ya no usamos replace(0, np.nan). Si el jefe puso un 0 en INCLS, se queda como 0.
df_final["fte incls"] = df_final["RUT_KEY"].map(dict_fte_incl)

# FTE REAL: Solo usa el Teórico si la inclusión es realmente "nula/vacía"
...
try:
    ...
    print(f"\n✅ REPORTE EXACTO CREADO CON ÉXITO.\nEl FTE Real será exactamente el de tu jefe.\nEl archivo está en:\n{ruta}")
