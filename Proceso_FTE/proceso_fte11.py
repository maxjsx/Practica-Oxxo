...
print("🚀 Iniciando FTE V10.0 (Jerarquía de Cargos SAP -> Talana)...")
...
# NUEVO: Diccionario con los cargos de SAP
...
# ==========================================
# 5. CONSTRUCCIÓN DE LA SÁBANA (LÓGICA FALLBACK)
# ==========================================
print("Cruzando Asistencia y calculando FTE...")
# Nota: Asumimos que la lista de Talana viene en GestiondeAsistencia.xlsx
...
# --- INICIO LÓGICA DE FALLBACK DE CARGO ---
# --- LÓGICA DE FALLBACK CORREGIDA ---
df_final["CARGO_SAP"] = df_final["RUT_KEY"].map(dict_cargo_sap)

# Si Talana dice que es PT, priorizamos Talana porque SAP suele ser genérico.
# Si no es PT en Talana, usamos SAP. Si SAP no existe, usamos Talana normal.
def definir_cargo_final(row):
    cargo_talana = str(row["Cargo"]).upper()
    cargo_sap = str(row["CARGO_SAP"]).upper() if pd.notna(row["CARGO_SAP"]) else ""
    
    if "PT" in cargo_talana or "PART TIME" in cargo_talana:
        return cargo_talana
    elif cargo_sap != "":
        return cargo_sap
    else:
...
df_final["CARGO OFICIAL"] = df_final.apply(definir_cargo_final, axis=1)

# ------------------------------------------

# Ahora usamos el CARGO OFICIAL para mapear y calcular el FTE
...
try:
    ...
    # Se eliminó RUT_KEY para que el detalle se vea más limpio
    ...
    print(f"\n✅ REPORTE CON FALLBACK DE CARGOS CREADO CON ÉXITO.\nEl archivo está en:\n{ruta}")
