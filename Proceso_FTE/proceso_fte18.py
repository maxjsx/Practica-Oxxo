...
def clean_text(x):
    if pd.isna(x): return ""
    return re.sub(r'\s+', ' ', str(x).strip().upper()) # Limpia dobles espacios
...
print("🚀 Iniciando FTE V17.0 (Lógica Pura y Precisa)...")
...
# FILTRO PERFECCIONADO: Licencias, Post Natal y Accidentes > 15 días (Protege las Vacaciones)
...
# Devolvemos la regla que ignora los 0s erróneos de la Inclusión
