...
print("🚀 Iniciando conciliación de FTE y Dashboard V8.0...")
...
# --- TABLA 2: RESUMEN GENERAL (HC Y FTE - AJUSTADO) ---
# Ahora suma los valores de la tabla Resumen directamente, simulando la Tabla Dinámica
...
# ==========================================
# 7. EXPORTACIÓN CON TABLAS NATIVAS Y FILTROS
# ==========================================
print("Pintando Excel y agregando Filtros Nativos...")
...
try:
    ...
    # --- 1. CREAR TABLA PRINCIPAL (CON FILTROS AUTOMÁTICOS) ---
    resumen.to_excel(writer, sheet_name="Resumen Tiendas", index=False, header=False, startrow=1, startcol=0)
    
    col_settings_main = [{'header': c} for c in resumen.columns]
    ws_resumen.add_table(0, 0, len(resumen), len(resumen.columns) - 1, {
        'columns': col_settings_main,
        'name': 'TablaPrincipal',
        'style': 'Table Style Medium 2' # Azul Corporativo Excel
    })

    # --- 2. CREAR TABLA DE RESUMEN GENERAL ---
    resumen_general.to_excel(writer, sheet_name="Resumen Tiendas", index=False, header=False, startrow=2, startcol=14)
    
    col_settings_gen = [{'header': c} for c in resumen_general.columns]
    ws_resumen.add_table(1, 14, 1 + len(resumen_general), 15, {
        'columns': col_settings_gen,
        'name': 'TablaGeneral',
        'style': 'Table Style Dark 11' # Verde Oscuro
    })
    ...
    col_settings_est = [{'header': c} for c in resumen_estado.columns]
    ws_resumen.add_table(6, 14, 6 + len(resumen_estado), 16, {
        'columns': col_settings_est,
        'name': 'TablaEstados',
        'style': 'Table Style Medium 3' 
    })

    # --- FORMATOS NUMÉRICOS Y ANCHOS DE COLUMNA ---
    ...
    # Forzar decimales en el total de FTE de la tabla pequeña
    ...
    print(f"\n✅ DASHBOARD GERENCIAL CREADO.\nEl archivo tiene filtros nativos y está en:\n{ruta}")
