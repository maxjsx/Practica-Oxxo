...
def to_float_safe(val):
    if pd.isna(val) or str(val).strip() == "": 
        return 0.0 # Cambiado a 0 para cálculos matemáticos precisos en brechas
...
print("🚀 Iniciando conciliación de FTE, Dashboard y Análisis de Brechas V9.0...")

# ==========================================
# 2. CARGA DE MAESTRA DE TIENDAS (AUTORIZADO)
# ==========================================
print("Cargando matriz de tiendas y cupos por cargo...")
...
# ==========================================
print("Cruzando Asistencia y calculando FTE...")
...
# ==========================================
print("Generando Resúmenes y Análisis de Brechas...")
...
# --- 6.1 LÓGICA DE BRECHAS POR CARGO ---
# Traductor de Nombres (Columna en Excel Autorizado -> Nombre Oficial en Agrupador)
map_cols_brecha = {
    "LIDER": "LIDER",
    "JEFE DE SALA": "JEFE DE SALA",
    "CAJERO": "CAJERO",
    "CAJERO PT 30": "CAJERO PT 30",
    "PT25": "CAJERO PT 25",
    "CAJERP PT 20": "CAJERO PT 20",  # Corrige el error de tipeo de la matriz
    "PT15": "CAJERO PT 15"
}

# Calculamos el Headcount REAL por Tienda y por Cargo Agrupado
real_cargos = df_final.groupby(['Ceco', 'Agrupador'])['Identificador'].nunique().unstack(fill_value=0)

# Construimos la tabla final de Brechas
brechas_list = []
df_aut_filtrado = df_aut[df_aut["CECO"].isin(resumen["CECO"])].copy() # Solo tiendas activas del reporte
...
for index, row in df_aut_filtrado.iterrows():
    ...
    for col_meta, nombre_agrupador in map_cols_brecha.items():
        # Cuántos pide el Excel de Metas
        meta = to_float_safe(row.get(col_meta, 0))
        # Cuántos tenemos realmente trabajando (si existe ese ceco y ese cargo)
        ...
        # Guardamos en columnas
        fila_brecha[f"Meta {nombre_agrupador}"] = meta
        fila_brecha[f"Real {nombre_agrupador}"] = real
        fila_brecha[f"VACANTES {nombre_agrupador}"] = meta - real # Positivo = Faltan (Vacante), Negativo = Sobran
...
try:
    ...
    fmt_sobre = workbook.add_format({'bg_color': '#DDEBF7', 'font_color': '#203764'}) # Azul

    # Formatos Hoja Resumen
    ...
    # Formatos Hoja Brechas
    ws_brechas.set_column('A:A', 10, formato_centrado)
    ws_brechas.set_column('B:B', 30) # Tienda
    ws_brechas.set_column('C:W', 18, formato_entero) # Columnas de Meta/Real/Vacantes
    
    # Pintar las columnas de "VACANTES" (Rojo si falta gente > 0, Verde si sobra o está cuadrado <= 0)
    for col_idx in range(4, len(df_brechas.columns), 3): # Salta de a 3 para caer justo en las columnas VACANTES
    ...
    print(f"\n✅ DASHBOARD Y BRECHAS CREADOS CON ÉXITO.\nEl archivo está listo en:\n{ruta}")
