...
def clean_ceco(x):
    if pd.isna(x): return ""
    s = str(x).strip().upper()
    if s.endswith(".0"): s = s[:-2] # Evita que 50096 se lea como 50096.0
    return s
...
print("🚀 Iniciando FTE V-FINAL (Filtro Tiendas Cerradas + Alineación)...")
...
# FIX 1: Identificar columna de estado y FILTRAR SOLO TIENDAS ABIERTAS
col_estado = [c for c in df_aut.columns if "ESTADO" in c][0]
df_aut = df_aut[df_aut[col_estado].astype(str).str.strip().str.upper() == "ABIERTA"].copy()

df_aut["CECO"] = df_aut["CECO"].apply(clean_ceco)
df_aut["NOMBRE_KEY"] = df_aut["NOMBRE MAESTRA"].apply(clean_text)
df_aut = df_aut.drop_duplicates(subset=["CECO"]) # Seguridad adicional
...
df_incl["Identificador_Raw"] = df_incl["Identificador"].astype(str)
df_incl = df_incl.drop_duplicates("Identificador_Raw")
dict_fte_incl_pq = dict(zip(df_incl["Identificador_Raw"], df_incl["FTE INCLS"].apply(to_float_safe)))

# ==========================================
# 4. CARGA DE SAP Y TALANA
# ==========================================
...
# ==========================================
print("Ejecutando cruces y filtrando Tiendas Cerradas...")
...
# Al mapear, cualquier tienda cerrada quedará como NaN (porque ya la borramos de dict_ceco)
df_ga["CECO_1"] = df_ga["Grupo"].apply(clean_text).map(dict_ceco)

# Esto elimina de cuajo a todo el personal de las tiendas cerradas
...
# Simular fragilidad INCLS de PQ
df_final["Identificador_Raw"] = df_final["Identificador"].astype(str)
df_final["FTE INCLS"] = df_final["Identificador_Raw"].map(dict_fte_incl_pq)

def calcular_fte_real(row):
    if row["Dias 2"] > 15:
        return 0.0
    elif row["FTE INCLS"] == 0.0:
        return 0.0
    else:
...
# ==========================================
# 6. RESUMEN EJECUTIVO Y ALINEACIÓN (EL FIX DE LA ILUSIÓN ÓPTICA)
# ==========================================
print("Generando Tablas Dinámicas y Brechas...")

# Agrupamos los datos reales calculados
agg_df = df_final.groupby("CECO_1").agg(HC=("Identificador", "nunique"), FTE_R=("FTE REAL", "sum")).reset_index()
agg_df = agg_df.rename(columns={"CECO_1": "CECO"})

# Construimos el esqueleto en base a las tiendas Abiertas
resumen = df_aut[["CECO", "NOMBRE MAESTRA", "JEFE OPERACIONES", "ASESOR TIENDA", "RECLUTADOR", "FTE AUT"]].copy()
resumen = resumen.rename(columns={
    "NOMBRE MAESTRA": "TIENDA", 
    "JEFE OPERACIONES": "JEFE DISTRITO", 
    "RECLUTADOR": "ENCARGADO RECLUTAMIENTO", 
    "FTE AUT": "FTE AUTORIZADO"
})

# Cruzamos (Left join para no perder el orden)
resumen = pd.merge(resumen, agg_df, on="CECO", how="left")

# Rellenamos nulos (tiendas abiertas sin asistencia)
resumen["HC"] = resumen["HC"].fillna(0)
resumen["FTE_R"] = resumen["FTE_R"].fillna(0)
resumen["FTE AUTORIZADO"] = resumen["FTE AUTORIZADO"].apply(to_float_safe).fillna(0)
...
resumen = resumen.rename(columns={"FTE_R": "FTE R"})
...
resumen = resumen[cols_res]

# FIX 2: ALINEACIÓN. Ordenamos por CECO A-Z para que cuadre exacto con Power Query
resumen = resumen.sort_values("CECO")
...
for index, row in df_aut.iterrows():
    ceco = row["CECO"]
...
# Alineamos las brechas al mismo orden A-Z
df_brechas = df_brechas.sort_values("CECO")
...
try:
    ...
    df_export = df_final.drop(columns=["RUT_SAP", "RUT_TALANA", "CARGO_UPPER", "Mantener", "FECHA_PARSEADA", "CECO_1", "AGRUPA_CARGO.AGRUPADOR", "AGRUPADOR_RESCATE", "Identificador_Raw"])
