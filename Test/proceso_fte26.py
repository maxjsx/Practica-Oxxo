...
print("🚀 Iniciando FTE V21.0 (Regla de Vacaciones - ABRIL 2026)...")
...
# ==========================================
# 3. CARGA DE AGRUPADOR (DOBLE SALTO EXACTO)
# ==========================================
print("Cargando Agrupador...")
df_agr = pd.read_excel("Agrupador 10.xlsx", sheet_name="AGRUPADOR", header=4)

# Índices directos para evitar confusión con columnas duplicadas
...
# Dict 1: Cargo -> Agrupador Real
dict_cargo_agrup = dict(zip(cargo_col, agrupador_col))

# Dict 2: Agrupador Real -> FTE Teorico
...
# --- REGLA DE ORO: PERMISOS Y VACACIONES ---
...
# 1. Filtramos: Eliminamos las "Vacaciones" de la lista de permisos que van a restar FTE.
# De esta forma, si alguien tiene vacaciones, para el script es como si no tuviera permisos, 
# y por lo tanto, nunca caerá en la regla de "> 15 días = 0".
df_perm_filtrado = df_perm[~df_perm["Tipo Permiso"].str.contains("Vacaciones", case=False, na=False)].copy()

df_perm_filtrado["F_INI"] = pd.to_datetime(df_perm_filtrado["Fecha Inicio"], dayfirst=True)
df_perm_filtrado["F_FIN"] = pd.to_datetime(df_perm_filtrado["Fecha Fin"], dayfirst=True)
df_perm_filtrado["DIAS"] = (df_perm_filtrado["F_FIN"] - df_perm_filtrado["F_INI"]).dt.days + 1

# Si hay múltiples permisos (ej. dos licencias distintas), tomamos la más larga para evaluar
df_perm_filtrado = df_perm_filtrado.sort_values(by="DIAS", ascending=False)
