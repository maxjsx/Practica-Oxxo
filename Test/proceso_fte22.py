...
# ==========================================
# 1. CONFIGURACIÓN Y FUNCIONES (PQ CLONE)
# ==========================================
...
def clean_rut_sap(x):
    if pd.isna(x): return ""
    # "Text.PadStart(Text.From([Identificador]), 12, "0")"
...
print("🚀 Iniciando FTE V20.0 (Clon Exacto Power Query M Code)...")
...
# ==========================================
# 3. CARGA DE AGRUPADOR (DOBLE SALTO)
# ==========================================
print("Cargando Agrupador (Doble Salto)...")
df_agr = pd.read_excel("Agrupador 10.xlsx", sheet_name="AGRUPADOR", header=4)
# Renombrar columnas duplicadas por si acaso
...
# Dict 1: Cargo -> Agrupador
dict_cargo_agrup = dict(zip(df_agr["CARGO"].apply(clean_text), df_agr["AGRUPADOR_1"].apply(clean_text) if "AGRUPADOR_1" in df_agr.columns else df_agr["AGRUPADOR"].apply(clean_text)))

# Dict 2: Agrupador -> FTE (Usamos drop_duplicates para simular el LeftOuterJoin de PQ)
...
df_incl["RUT_KEY"] = df_incl["IDENTIFICADOR"].apply(clean_rut_talana)
...
# "Duration.Days([Fecha Fin] - [Fecha Inicio]) + 1"
df_perm["DIAS"] = (df_perm["F_FIN"] - df_perm["F_INI"]).dt.days + 1
dict_dias_permiso = dict(zip(df_perm["RUT_TALANA"], df_perm["DIAS"]))

# ==========================================
# 5. CONSTRUCCIÓN DE LA SÁBANA (LÓGICA M)
# ==========================================
print("Ejecutando cruces de Power Query...")
...
# Filtrar nulos en CECO_1
df_final = df_ga[df_ga["CECO_1"].notna()].copy()

# Traer fechas para Bajas
def parse_fecha(x):
    try:
...
# DOBLE SALTO DE AGRUPADOR (M Code Simulation)
...
# Si es nulo, no mapea nada extra, pero evitamos error
df_final["AGRUPA_CARGO.FTE TEORICO"] = df_final["AGRUPA_CARGO.AGRUPADOR"].fillna(df_final["CARGO_UPPER"]).map(dict_agrup_fte).fillna(0.0)

# Permisos y FTE REAL ("if [Dias 2] > 15 then 0 else if INCLS=0 then 0 else TEORICO")
...
def calcular_fte_real(row):
    if row["Dias 2"] > 15:
        ...
        # El PQ IGNCORA los valores decimales de INCLS (ej. 0.68) y asume TEORICO directo.
        return row["AGRUPA_CARGO.FTE TEORICO"]

df_final["FTE REAL"] = df_final.apply(calcular_fte_real, axis=1)

# Quitar Duplicados (Paso #"Duplicados quitados4")
...
real_cargos = df_final.groupby(['CECO_1', 'AGRUPA_CARGO.AGRUPADOR'])['Identificador'].nunique().unstack(fill_value=0)
brechas_list = []
...
try:
    ...
    # Exportamos el detalle limpio
    df_export = df_final.drop(columns=["RUT_SAP", "RUT_TALANA", "CARGO_UPPER", "Mantener", "FECHA_PARSEADA", "CECO_1", "AGRUPA_CARGO.AGRUPADOR"])
