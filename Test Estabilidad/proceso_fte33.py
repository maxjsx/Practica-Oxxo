...
# --- NUEVA FUNCIÓN: BUSCADOR AUTOMÁTICO DE ARCHIVOS ---
def buscar_archivo(palabra_clave):
    archivos = [f for f in os.listdir('.') if palabra_clave.lower() in f.lower() and f.endswith(('.xlsx', '.xls')) and not f.startswith('~')]
    if not archivos:
        raise FileNotFoundError(f"\n❌ ERROR: No se encontró ningún archivo Excel que contenga la palabra '{palabra_clave}' en esta carpeta.")
...
print("🚀 Iniciando FTE ESTABLE (Auto-Detección de Archivos)...")

# ==========================================
# 2. CARGA DE MAESTRA DE TIENDAS
# ==========================================
print("\nCargando matriz de tiendas...")
# Busca automáticamente el archivo que contenga "FTE AUTORIZADO"
df_aut = pd.read_excel(buscar_archivo("FTE AUTORIZADO"), sheet_name="ABRIL_26", header=6)
df_aut.columns = [str(c).strip().upper() for c in df_aut.columns]

df_aut["CECO"] = df_aut["CECO"].astype(str).str.strip().str.upper()
...
# ==========================================
print("\nCargando Agrupador y Diccionarios...")
archivo_agrupador = buscar_archivo("Agrupador")

df_agr = pd.read_excel(archivo_agrupador, sheet_name="AGRUPADOR", header=4)
...
df_incl = pd.read_excel(archivo_agrupador, sheet_name="INCLS", header=3)
...
# ==========================================
print("\nProcesando SAP, Bajas y Permisos...")

df_act = pd.read_excel(buscar_archivo("Activos"))
...
df_talana_emp = pd.read_excel(buscar_archivo("Lista Empleados"))
...
df_bajas = pd.read_excel(buscar_archivo("Bajas"))
...
df_perm = pd.read_excel(buscar_archivo("Permisos"))
...
df_perm_unique = df_perm.drop_duplicates(subset=["RUT_TALANA"], keep='first')
...
# ==========================================
print("\nEjecutando cruces...")
df_ga = pd.read_excel(buscar_archivo("Asistencia"), header=1)
...
df_final["CECO_1"] = df_final["CECO_1"].astype(str)
...
# ==========================================
print("\nGenerando Tablas Dinámicas y Brechas...")
...
# ==========================================
print("\nPintando Excel y generando Tablas Oficiales...")
